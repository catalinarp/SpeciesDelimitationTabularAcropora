### Model-based genetic clustering of available molecular Sanger Sequenced markers ###
# Following tutorial from: http://www.royfrancis.com/pophelper/articles/index.html#input-files
# Includes: Structure output for markers FZD and PMCA

# 1. Set working directory
setwd("~/Projects/Speciesdelimitation_tabularAcropora/MolecularAnalyses/Structure")

# 2. Load packages:
library(pophelper)
library(gridExtra)
library(strataG)
library(readr)
library(gplots)
library(ggplot2)
library(RColorBrewer)
library(pophelper)

## NOTE: In the bash shell, remember to run in each folder the following lines to put all the output files for the different K values in the same folder:
#cd STR1_2loci
#mkdir results_f/all_k/;
#for i in results_f/k*/*; do cp $i results_f/all_k; done

# 3. Load dataset:
## STR1: Admixture and correlated frequencies and no prior:
filenames1 <- list.files(path = "STR1_2loci/results_f/all_k/", pattern = "input2lociSS",full.names = TRUE)
slist1 <- readQ(files = filenames1, filetype = "structure")
dir.names <- list.dirs(full.names = TRUE)

# 4. Create a table and a summary (this has to be done for each set of parameters if >1)
tr1 <- tabulateQ(qlist=slist1, writetable=TRUE,exportpath = dir.names[2])
sm1 <- summariseQ(tr1, writetable=TRUE, exportpath = dir.names[2])

# 5. Calculate best K according to Evanno (delta K) method
tr1 <- tabulateQ(qlist=slist1, writetable=F)
sm1 <- summariseQ(tr1, writetable=F)
p <- evannoMethodStructure(data=sm1,exportplot=F,returnplot=T,returndata=F,basesize=12,linesize=0.7)
grid.arrange(p)

# 6. Use Pophelper + CLUMPP to combine, align and merge runs for each K:
clumpComb1 <- clumppExport(slist1) # To combine str files and generate clumpp input
# NOTE: Remember to move them to the appropiate directory in: results_f/all_k/

# NOTE: In the command line, run CLUMPP for each K you want to plot:
# Example: /Academic_software/CLUMPP_MacOSX.1.1.2/CLUMPP paramfile
# Now load data from CLUMPP output for the most likely best Ks:

clumpAln1K3 <- readQClumpp("STR1_2loci/results_f/all_k/pop_K3/pop_K3-combined-aligned.txt")
clumpAln1K2 <- readQClumpp("STR1_2loci/results_f/all_k/pop_K2/pop_K2-combined-aligned.txt")

# 7. Load metadata (individual labels):
metadata <- read.delim("LabelsInd.txt", sep="\t", header = FALSE,stringsAsFactors=F)
rownames(clumpAln1K3[[1]]) <- metadata$V1
rownames(clumpAln1K2[[1]]) <- metadata$V1
morphogroup <- metadata[,3,drop=F]

# 8. Check the aligned plots first to see they all convey the same info. 
# NOTE: Use only to visualize, you can delete this preliminary plots after
p1k2 <- plotQMultiline(clumpAln1K2,spl=36, height=8, width=14, useindlab=T,lpp=1,
                       sortind="Cluster2",barsize=1, grplab=morphogroup,
                       clustercol = brewer.pal(2,'YlGnBu'), ordergrp=TRUE)
p1k3 <- plotQMultiline(clumpAln1K3,spl=36, height=8, width=14, useindlab=T,lpp=1,
                       sortind="Cluster3",barsize=1, grplab=morphogroup,
                       clustercol = brewer.pal(3,'YlGnBu'), ordergrp=TRUE)
p1k4 <- plotQMultiline(clumpAln1K4,spl=36, height=8, width=14, useindlab=T,lpp=1,
                       sortind="Cluster3",barsize=1, grplab=morphogroup,
                       clustercol = brewer.pal(3,'YlGnBu'), ordergrp=TRUE)

# 9. Once that has been done, use the merged CLUMPP files to generate the final plots:

clumpMer1K3 <- readQClumpp("STR1_2loci/results_f/all_k/pop_K3/pop_K3-combined-merged.txt")
clumpMer1K2 <- readQClumpp("STR1_2loci/results_f/all_k/pop_K2/pop_K2-combined-merged.txt")
clumpMer1K4 <- readQClumpp("STR1_2loci/results_f/all_k/pop_K4/pop_K4-combined-merged.txt")

rownames(clumpMer1K2[[1]]) <- metadata$V1
rownames(clumpMer1K3[[1]]) <- metadata$V1
rownames(clumpMer1K4[[1]]) <- metadata$V1
morphogroup <- metadata[,3,drop=F]

# NOTE: Build color sets according to the number of Ks.
# Examples: 
colorset2 <- c("#ce882a", "#7e6729")
colorset2 <- c("goldenrod","royalblue4")
colorset3 <- c("#ce882a", "#7e6729","#d5af29")
colorset3 <- c("#FBA90A","#2c5ab4","#009933")
colorset4 <- c("cyan4","goldenrod","forestgreen","royalblue4")

# Now, build the plots:
# Example with K=3
p1k3 <-  plotQMultiline(clumpMer1K3,spl=36, height=8, width=14, useindlab=T,subtitlelab="k=3",
                        lpp=1,barsize=1, grplab=morphogroup,showsubtitle=T, showgrplab = T,
                        clustercol = colorset3, showyaxis = T, subtitlesize=8,ordergrp=TRUE,
                        barbordersize = 0.5, barbordercolour="white",subtitlehjust = 1,
                        indlabangle=90, indlabvjust = 0.9,indlabhjust = 0.5,indlabsize=8,
                        sortind ="label",returnplot=T,exportplot=F)
grid.arrange(p1k3$plot[[1]][[1]],nrow=1)

# Example with K=2
p1k2 <-  plotQMultiline(clumpMer1K2,spl=36, height=8, width=14, useindlab=T,subtitlelab="k=2",
                        lpp=1,barsize=1, grplab=morphogroup,showsubtitle=T, showgrplab = T,
                        clustercol = colorset2, showyaxis = T, subtitlesize=8,ordergrp=TRUE,
                        barbordersize = 0.5, barbordercolour="white",subtitlehjust = 1,
                        indlabangle=90, indlabvjust = 0.9,indlabhjust = 0.5,indlabsize=8,
                        sortind ="label",returnplot=T,exportplot=F)
grid.arrange(p1k2$plot[[1]][[1]],nrow=1)

# Example with K=4
p1k4 <-  plotQMultiline(clumpMer1K4,spl=36, height=8, width=14, useindlab=T,subtitlelab="k=2",
                        lpp=1,barsize=1, grplab=morphogroup,showsubtitle=T, showgrplab = T,
                        clustercol = colorset4, showyaxis = T, subtitlesize=8,ordergrp=TRUE,
                        barbordersize = 0.5, barbordercolour="white",subtitlehjust = 1,
                        indlabangle=90, indlabvjust = 0.9,indlabhjust = 0.5,indlabsize=8,
                        sortind ="label",returnplot=T,exportplot=F)
grid.arrange(p1k4$plot[[1]][[1]],nrow=1)

# Example of plot including k=2, k=3, and k=4
grid.arrange(p1k2$plot[[1]][[1]],p1k3$plot[[1]][[1]],p1k4$plot[[1]][[1]], nrow=3)

